...
func queryCreateMatch(ctx context.Context, tx pgx.Tx, round_number int, tournamentId, team1Id, team2Id int64) error {

	const sql = `
		WITH

		new_match AS (
				INSERT INTO match (team1_id, team2_id)
				VALUES (
						($1),	($2)
				)
				RETURNING id
	...
	var id int64
	if err := tx.QueryRow(ctx, sql, team1Id, team2Id, tournamentId, round_number).Scan(&id); err != nil {
...
func CreateTournament(ctx context.Context, conn *pgxpool.Pool, t *tournament.Tournament) error {
	...
	for roundIndex, round := range rounds {
		for _, match := range round {
			...
			err := queryCreateMatch(ctx, tx, roundIndex, tournamentId, team1Id, team2Id)
			if err != nil {
				return err
			}
		}
